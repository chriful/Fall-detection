...
#get_ipython().system('mkdir -p  /data1/Wch/ten_animal/demo')
#get_ipython().system('wget "http://ai-atest.bj.bcebos.com/cifar-10-python.tar.gz" -O cifar-10-python.tar.gz')
#get_ipython().system('mv cifar-10-python.tar.gz  /data1/Wch/ten_animal/demo')
#get_ipython().system('ls -a /data1/Wch/ten_animal/demo')
...
# *** **CNN网络模型**
# 
# 在CNN模型中，卷积神经网络能够更好的利用图像的结构信息。下面定义了一个较简单的卷积神经网络。显示了其结构：输入的二维图像，先经过三次卷积层、池化层和Batchnorm，再经过全连接层，最后使用分类作为输出层。
# 
...
train_reader = paddle.batch(paddle.reader.shuffle(train_reader(crop_size, resize_size),buf_size=60000),batch_size=BATCH_SIZE)
