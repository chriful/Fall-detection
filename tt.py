...
def showImg():
    ...
    cap = cv2.VideoCapture("D:\\fsdownload\\human_pose_estimation-11\\video\\1.mp4")
    while 1:
#        time.sleep(0.1)
        if start==1:
            ret,frame = cap.read()
...
#        cv2.imwrite("/home/data3/fall/human_pose_estimation/test/004.jpg", frame)
            if flag is 0:
#        		
                cv2.imwrite("D:\\fsdownload\\human_pose_estimation-11\\test\\004.jpg", frame)
    ...
    while ff==0:
        if flag is 1:
            ...
            for batch_id, data in enumerate(test_reader()):
                ...
                cod = save_predict_results(input_image, out_heatmaps, file_ids, fold_name='results')
                if j==1:
                    offset = 0
                    cos=cod
                else:
            ...
            img = cv2.imread("D:\\fsdownload\\human_pose_estimation-11\\results\\rendered_0000004.png")
            if monitor > 0:
