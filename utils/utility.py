...
def save_predict_results(batch_image, batch_heatmaps, file_ids, fold_name, normalize=True):
    ...
    # (32, 16, 2), (32, 16, 1))
    preds, maxvals = get_max_preds(batch_heatmaps)
    # Center of gravity of key points
    # weight = np.array([[0.0634], [0.1630], [0.1630], [0.0634], [0.1879], [0.2391], [0.0706], [0.0192], [0.0580], [0.0356], [0.0356], [0.0580], [0.0192]])
    weight = np.array(
        [[0.0317], [0.0815], [0.06485], [0.06485], [0.0815], [0.0317], [0.1879], [0.2391], [0.0706], [0.0096], [0.029],
         [0.0178], [0.0178], [0.029], [0.0096]])
    
    
    
    key = preds[0]
    key = np.delete(key, 8, axis=0)
    key[7][0] = (key[12][0] + key[13][0]) / 2
    key[7][1] = (key[12][1] + key[13][1]) / 2
    key[6][0] = (key[2][0] + key[3][0]) / 2
    key[6][1] = (key[2][1] + key[3][1]) / 2

    # print(weight.shape)
    # print(weight)
    # print(np.transpose(key).shape)
    # print(np.transpose(key))
    gravity = np.matmul(np.transpose(key), weight)

    #t=-1时重心低于盆骨
    t=-1
    if gravity[1][0] - key[6][1] > 0:
        t=1

    #重心到盆骨的相对距离
    coordinate = t*((((gravity[0][0] - key[6][0])**2) + ((gravity[1][0] - key[6][1])**2))**0.5)

    # 左手腕和右手腕到头距离的最小值
    rhh = math.sqrt((preds[0][10][0] - preds[0][9][0]) ** 2 + (preds[0][10][1] - preds[0][9][1]) ** 2)
    lhh = math.sqrt((preds[0][15][0] - preds[0][9][0]) ** 2 + (preds[0][15][1] - preds[0][9][1]) ** 2)
    if rhh > lhh:
        hh = lhh
    else:
        hh = rhh
    # 左手腕和右手腕到胸腔距离的最小值
    rht = math.sqrt((preds[0][10][0] - preds[0][7][0]) ** 2 + (preds[0][10][1] - preds[0][7][1]) ** 2)
    lht = math.sqrt((preds[0][15][0] - preds[0][7][0]) ** 2 + (preds[0][15][1] - preds[0][7][1]) ** 2)
    if rht > lht:
        ht = lht
    else:
        ht = rht
    ...
    for i in range(batch_size):
        ...
        for j in range(num_joints):
            ...
            if j == 0 or j == 6 or j == 10:
                continue
            cv2.line(image, (int(x1 * 4), int(y1 * 4)), (int(x * 4), int(y * 4)), (0, 255, 0), 1)
        cv2.circle(image, (int(gravity[0][0] * 4), int(gravity[1][0] * 4)), 4, gcolor, -1, 16)
        cv2.imwrite(str(imgname), image)

        
    return coordinate, hh, ht
